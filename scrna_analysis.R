...
sample1 <- readRDS(
  file.path(data_dir, "GSM4557329_GSM4557329_556_cell.counts.matrices.rds")
)

sample2 <- readRDS(
  file.path(data_dir, "GSM4557330_GSM4557330_557_cell.counts.matrices.rds")
)

sample3 <- readRDS(
  file.path(data_dir, "GSM4557331_GSM4557331_558_cell.counts.matrices.rds")
)

sample4 <- readRDS(
  file.path(data_dir, "GSM4557337_GSM4557337_HIP043_cell.counts.matrices.rds")
)
...
# Add percentage mitochondrial metric
...
seurat2[["percent.mt"]] <- PercentageFeatureSet(
  seurat2,
  pattern = "^MT-"
)

seurat3[["percent.mt"]] <- PercentageFeatureSet(
  seurat3,
  pattern = "^MT-"
)

seurat4[["percent.mt"]] <- PercentageFeatureSet(
  seurat4,
  pattern = "^MT-"
)
...
for (s in names(seurat_list)) {
  ...
  obj_clean <- obj[
    ,
    obj@meta.data[[df_col]] == "Singlet"
  ]

  message(
    "Remaining singlet cells: ",
    ncol(obj_clean)
  )
...
merged_noBC <- NormalizeData(
  merged_noBC
)

merged_noBC <- FindVariableFeatures(
  merged_noBC
)

merged_noBC <- ScaleData(
  merged_noBC
)
...
DimPlot(
  ...
  ggtitle(
    "A: UMAP WITHOUT Batch Correction"
  )
...
DimPlot(
  ...
  ggtitle(
    "UMAP WITH Batch Correction (LogNormalize Integration)"
  )
...
ElbowPlot(
  ...
  ggtitle(
    "Elbow Plot for Selecting Number of PCs"
  )


# Choose PCs based on elbow plot
...
# Prepare case-insensitive marker list

marker.list_lower <- lapply(
  marker.list,
...
marker_hits_per_type <- sapply(
  ...
  function(m)
    sum(
      m %in% gene_names_lower
    )
)

print(
  "marker hits per marker-set:"
)

print(
  marker_hits_per_type
)


# Prepare clusters and storage
...
for (cl in cluster_ids) {
  ...
  scores <- sapply(
    marker.list_lower,
    function(marker_genes_lower) {

      hits_idx <- which(
        gene_names_lower %in% marker_genes_lower
      )
...
print(
  "cluster -> chosen marker set (abbrev):"
)
...
# Pretty mapping from abbreviation to label
...
manual_labels_map <- setNames(
  cluster_pretty,
  cluster_ids
)


# Assign per-cell labels
...
print(
  table(
    integrated$manual_celltype,
    useNA = "ifany"
  )
)


# Plot manual annotation UMAP
...
DimPlot(
  ...
  ggtitle(
    "Manual Cell Type Annotation (Table 2 markers)"
  )


# Compare manual vs automatic annotation
...
wrap_plots(
  ...
  plot_annotation(
    title = "Expression of 3 marker genes"
  )
...
# Merge clusters into cell-type groups

Idents(integrated) <- integrated$manual_celltype

integrated$celltype_merged <- Idents(
  integrated
)

message(
  "Finished manual annotation and merging."
)
...
newRNA <- CreateAssayObject(
  counts = raw_counts_clean
)
...
# Recreate normalized data

integrated <- NormalizeData(
  integrated
)

integrated <- FindVariableFeatures(
  integrated
)

message(
  "RNA assay cleaned and normalized successfully."
)
...
volcano_b_t <- ggplot(
  ...
  ggtitle(
    "Volcano Plot: B cells vs T cells"
  ) +
...
volcano_t_mono <- ggplot(
  ...
  ggtitle(
    "Volcano Plot: T cells vs Monocyte (CD16)"
  ) +
...
de_covid_vs_healthy$gene <- rownames(
  de_covid_vs_healthy
)
...
top_up <- head(
  de_covid_vs_healthy[
    order(
      -de_covid_vs_healthy$avg_log2FC
    ),
  ],
  10
)
...
if (
  !"p_val_adj" %in%
  colnames(de_t_vs_mono) &
  "p_val" %in%
  colnames(de_t_vs_mono)
) {

  de_t_vs_mono$p_val_adj <-
    de_t_vs_mono$p_val
}
...
de_t_vs_mono <- de_t_vs_mono %>%
  ...
  mutate(
    ...
    enriched_in = ifelse(
      avg_log2FC > 0,
      "T cell (T)",
      "Monocyte CD16 (Mono16)"
    )
  )


# Select top 5 by adjusted p-value
...
p_dot <- ggplot(
  ...
  xlab(
    "Cell type where the gene is enriched"
  ) +

  ylab(
    "Top DE genes (per comparison)"
  ) +

  ggtitle(
    "Top 5 DE genes per comparison (dotplot)"
  )
...
ggplot(
  ...
  ggtitle(
    "Top GO Biological Processes (T cells vs B cells)"
  )
